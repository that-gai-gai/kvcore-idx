...
class kvCORE_AdminPageFramework_Form_View___Script_RepeatableSection extends kvCORE_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
     ...
     */
    $.fn.updatekvCORE_AdminPageFrameworkRepeatableSections = function( aSettings ) {
    
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.kvcore-sections' ).attr( 'id' );
    
        // Store the sections specific options in an array.
        if ( ! $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {    
        ...
        var _aOptions = $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).click( function() {
            ...
            $( this ).addkvCORE_AdminPageFrameworkRepeatableSection();
        ...
        $( _oThis ).find( '.repeatable-section-remove-button' ).click( function() {
            $( this ).removekvCORE_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });     
        
        // If the number of sections is less than the set minimum value, add sections. 
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.kvcore-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.kvcore-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {     
                $( '#' + _sSectionID ).addkvCORE_AdminPageFrameworkRepeatableSection( _sSectionID );  
            }
        }
     ...
     */    
    $.fn.addkvCORE_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        
        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.kvcore-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.kvcore-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.kvcore-section-tabs-contents' )
            .children( '.kvcore-section-tabs' )
            .first();
            
        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );
        
        var _iFadein                = $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        
        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.kvcore-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.kvcore-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.kvcore-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.kvcore-section-title' ).not( '.kvcore-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enablekvCORE_AdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.kvcore-collapsible-sections-title, .kvcore-collapsible-section-title' ).enablekvCORE_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.kvcore-field' ).each( function( iFieldIndex ) {
            ...
            // 3.8.8 
            $( this ).trigger( 
                'kvcore_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.kvcore-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.updatekvCORE_AdminPageFrameworkRepeatableSections();    
        
        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.kvcore-fields.sortable' ).each( function() {
            $( this ).enablekvCORE_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.kvcore-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.kvcore-fieldrow, .kvcore-fieldset, .kvcore-fields, .kvcore-field, table.form-table, input,textarea,select' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.kvcore-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.kvcore-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.kvcore-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removekvCORE_AdminPageFrameworkRepeatableSection = function() {
        
        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.kvcore-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.kvcore-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.kvcore-section-tabs-contents' )
            .children( '.kvcore-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.kvcore-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.akvCORE_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.kvcore-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.kvcore-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.kvcore-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.kvcore-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.kvcore-field' ).each( function( iFieldIndex ) {    
...
JAVASCRIPTS;
    ...
    static public function getEnabler($sContainerTagID, $iSectionCount, $asArguments, $oMsg) {
        ...
        new self($oMsg);
        $_oFormatter = new kvCORE_AdminPageFramework_Form_Model___Format_RepeatableSection($asArguments, $oMsg);
        $_aArguments = $_oFormatter->get();
        $_sButtons = "<div class='kvcore-repeatable-section-buttons-outer-container'>" . "<div " . self::___getContainerAttributes($_aArguments, $oMsg) . ' >' . "<a " . self::___getRemoveButtonAttributes($sContainerTagID, $oMsg, $iSectionCount) . ">-</a>" . "<a " . self::___getAddButtonAttributes($sContainerTagID, $oMsg, $_aArguments) . ">+</a>" . "</div>" . "</div>" . kvCORE_AdminPageFramework_Form_Utility::getModalForDisabledRepeatableElement('repeatable_section_disabled_' . $sContainerTagID, $_aArguments['disabled']);
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .kvcore-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.kvcore-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.kvcore-collapsible-section-title' );
    ...
    // Update the fields     
    jQuery( '#{$sContainerTagID}' ).updatekvCORE_AdminPageFrameworkRepeatableSections( $_aJSArray ); 
});            
JAVASCRIPTS;
        return "<script type='text/javascript' class='kvcore-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('kvcore-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
