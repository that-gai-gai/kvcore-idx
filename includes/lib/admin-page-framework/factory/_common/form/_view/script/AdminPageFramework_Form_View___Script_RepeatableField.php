...
class kvCORE_AdminPageFramework_Form_View___Script_RepeatableField extends kvCORE_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updatekvCORE_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ];
...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.kvcore-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.kvcore-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.kvcore-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.kvcore-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.         
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            ...
            $( this ).addkvCORE_AdminPageFrameworkRepeatableField();
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removekvCORE_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.kvcore-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.kvcore-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addkvCORE_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addkvCORE_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.kvcore-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.kvcore-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.kvcore-repeatable-field-buttons' );
            $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.kvcore-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.kvcore-repeatable-field-buttons' ).last();
         ...
         */
        nodeNewField.updatekvCORE_AdminPageFrameworkRepeatableFields();
        ...
        $( nodeNewField ).find( '.kvcore-field' ).addBack().trigger( 
            'kvcore_repeated_field', 
            [ 
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.kvcore-fields' )    // model container
            ]
        );
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.kvcore-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.kvcore-field,.kvcore-fields,.kvcore-fieldset' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removekvCORE_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.kvcore-field' );
        var nodeFieldsContainer = $( this ).closest( '.kvcore-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.kvcore-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.kvcore-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.akvCORE_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
