...
abstract class kvCORE_AdminPageFramework_PageLoadInfo_Base extends kvCORE_AdminPageFramework_FrameworkUtility {
    public $oProp;
    public $oMsg;
    ...
    private function _getPageLoadStats() {
        ...
        $_nMemoryUsage = round((int)$this->_convertBytesToHR(memory_get_usage()), 2);
        $_nMemoryPeakUsage = round((int)$this->_convertBytesToHR(memory_get_peak_usage()), 2);
        $_nMemoryLimit = round((int)$this->_convertBytesToHR($this->_convertToNumber(WP_MEMORY_LIMIT)), 2);
        $_sInitialMemoryUsage = round((int)$this->_convertBytesToHR($this->_nInitialMemoryUsage), 2);
        return "<div id='kvcore-page-load-stats'>" . "<ul>" . "<li>" . sprintf($this->oMsg->get('queries_in_seconds'), $_nQueryCount, $_nSeconds) . "</li>" . "<li>" . sprintf($this->oMsg->get('out_of_x_memory_used'), $_nMemoryUsage, $_nMemoryLimit, round(($_nMemoryUsage / $_nMemoryLimit), 2) * 100 . '%') . "</li>" . "<li>" . sprintf($this->oMsg->get('peak_memory_usage'), $_nMemoryPeakUsage) . "</li>" . "<li>" . sprintf($this->oMsg->get('initial_memory_usage'), $_sInitialMemoryUsage) . "</li>" . "</ul>" . "</div>";
    }
